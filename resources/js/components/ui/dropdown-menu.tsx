...
const DropdownMenu = (props: MenuTriggerProps) => (
    <MenuTrigger data-slot='dropdown-menu' {...props} />
)

const DropdownMenuTrigger = (props: ButtonProps) => (
...
const DropdownMenuContent = <T extends object>({
    ...
    Pick<
        PopoverProps,
        'offset' | 'crossOffset' | 'placement' | 'isOpen' | 'onOpenChange' | 'triggerRef'
    >) => (
    <Popover
        className={cn(
    ...
    >
        <Menu
            ...
            {...props}
            renderEmptyState={() => (
                <div className='rounded-sm px-2 py-1.5 text-muted-foreground text-sm'>No Data</div>
            )}
        />
    </Popover>
)

const DropdownMenuGroup = <T extends object>({
    title,
    ...props
}: MenuSectionProps<T> & { title?: string }) => (
    <MenuSection
        className={cn('flex flex-col text-sm', props.className)}
        data-slot='command-group'
        {...props}
    >
        {title && (
            <Header className='pointer-events-none px-2 py-1 font-medium text-muted-foreground text-xs'>
                {title}
            </Header>
        )}
    ...
    >
        {(values) => (
            <>
                {values.isSelected && (
                    ...
                    >
                        {values.selectionMode === 'single' && (
                            <IconCircleFilled
                                className='mx-1 size-2 text-current'
                ...
                {values.hasSubmenu && (
                    <IconChevronRight className='absolute right-2 size-3.5' data-slot='chevron' />
                )}
            </>
        )}
    </MenuItem>
...
const DropdownMenuSub = (props: SubmenuTriggerProps) => (
