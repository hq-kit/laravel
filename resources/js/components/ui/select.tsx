...
const SelectContent = <T extends object>({
    ...
    Pick<PopoverProps, 'offset'> & { side?: 'bottom' | 'top'; isSearchable?: boolean }) => {
    const renderContent = () => (
        <ListBox
            className='flex flex-col overflow-y-auto rounded-lg p-1 outline-hidden sm:max-h-[inherit]'
                    ...
                    <Autocomplete
                        filter={(textValue, inputValue) => {
                            ...
                            while (textIndex < textValue.length && inputIndex < inputValue.length) {
                                if (
                                    textValue.toLowerCase()[textIndex] ===
                                    inputValue.toLowerCase()[inputIndex]
                                ) {
                                    inputIndex++
                                }
                                textIndex++
...
const SelectGroup = <T extends object>({
    title,
    ...props
