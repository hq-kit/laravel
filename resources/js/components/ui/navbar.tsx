...
const NavbarSection = ({ className, ...props }: ComponentPropsWithRef<'div'>) => {
    ...
    return (
        <div
            className={cn(
                'flex',
                isCompact ? 'flex-col gap-y-1' : 'flex-row items-center gap-x-3',
                className,
            )}
            data-navbar-section
            {...props}
        >
            {'title' in props && (
                <h4 className='-mx-2 my-4 px-5 font-medium text-sm md:hidden'>{props.title}</h4>
            )}
            {props.children}
        </div>
    )
...
const navbarItemVariants = tv({
    base: 'peer/menu-button flex w-full select-none items-center gap-2 whitespace-nowrap rounded-md p-2 text-sm outline-hidden transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:border data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-focus-visible:ring-2 data-focus-visible:ring-sidebar-ring [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
    variants: {
        variant: {
...
const NavbarBreadcrumbs = ({
    ...
    return (
        <div
            ...
            {...props}
        >
            <div className={cn(variant === 'default' && 'mx-auto w-full px-4 sm:px-6 lg:px-8')}>
                {children}
            </div>
        </div>
    )
        ...
        >
            <main
                className={cn(
                    ...
                    variant === 'floating' &&
                        'max-w-7xl px-4 lg:max-w-(--breakpoint-xl) 2xl:max-w-(--breakpoint-2xl)',
                )}
            >
                {props.children}
            </main>
