...
const ColorWheel = ({
    ...
    innerRadius = 74,
    ...props
}: Omit<ColorWheelProps, 'innerRadius' | 'outerRadius'> & {
    innerRadius?: number
    outerRadius?: number
}) => (
    <RACColorWheel
...
const ColorArea = ({ className, ...props }: ColorAreaProps) => (
    <RACColorArea
        className={composeRenderProps(className, (className) =>
            cn(
                'size-48 shrink-0 rounded-md shadow-md data-disabled:opacity-50 data-disabled:grayscale-50',
                className,
            ),
        )}
        {...props}
...
const ColorSliderOutput = ({ className, ...props }: SliderOutputProps) => (
    <RACSliderOutput
        className={composeRenderProps(className, (className) =>
            cn(
                'in-data-[orientation=vertical]:hidden font-medium text-base/6 sm:text-sm/6',
                className,
            ),
        )}
        {...props}
