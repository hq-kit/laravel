...
class UserController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $users = User::query()
            ->with('roles')
            ->when($role_filter, fn ($q) => $q
                ->whereHas('roles', fn ($r) => $r->whereIn('name', $role_filter))
            )
            ...
            ->when($request->order, fn ($q, $col) => $q->orderBy($col, $request->direction === 'desc' ? 'desc' : 'asc'),
                fn ($q) => $q->latest()
            )
        ...
        return inertia('users/index', [
            ...
            'filters' => $request->only('search', 'order', 'direction', 'role'),
            'roles' => $roles,
        ]);
    }
    ...
    public function bulk(Request $request): RedirectResponse
    {
        ...
        if ($ids->isEmpty()) {
            inertia()->flash('toast', ['type' => 'error', 'message' => __('No users to process.')]);
        ...
        try {
            ...
            inertia()->flash('toast', ['type' => 'error', 'message' => __('Something went wrong')]);
    ...
    private function bulkDelete(Request $request): void
    {
